...
if((!is.null(input$filterCountLevel) && !is.null(input$filterCountN) &&
 !is.null(input$filterCountGroup) && input$doFilterCount == 1) ||
   (input$doProductive == 1) ||
    (!is.null(input$filterSeqLevel) && !is.null(input$filterSeqName) && input$doFilterSeq == 1) || 
...
if((!is.null(input$plotRare) && input$plotRare != "") ||
...
if(!is.null(input$plotRare) && input$plotRare != ""){cat("### Rarefaction analysis")}
```
...
if(input$renyiLevel != "" && input$renyicolor !=""){
   plotGenDiversity(x=dataFilt(), level = input$renyiLevel, colorBy = input$renyicolor,                Hill=input$multidoHill,
...
if(input$countIntLevel != "" && input$countIntGroupMeth != ""){
    plotIntervals(x=dataFilt(), level = input$countIntLevel, colorBy = NULL,
                  facetBy = input$countIntfacet,
                   calculation_type = input$countIntCalType,
                  interval_scale=input$countIntGroupMeth, grouped = F, label_colors = NULL)
...
```{r show stats text entry,echo = FALSE}
  if((!(is.null(input$plotStats) || input$plotStats == "")) &&
   !(is.null(input$statisticsGroup) || input$statisticsGroup == "")){
  tags$body(style="color: white;",
       p(style="color: white;",
...
if(!is.null(input$multrenLevel) && !is.null(input$multrenGroup)){
   ...
   #  }
    plotGenDiversity(x = dataFilt(), level = input$multrenLevel, colorBy = input$multrenGroup, facetBy=input$multrenfacet, Hill=input$multidoHill, grouped = TRUE, label_colors = NULL)
}
...
if(!is.null(input$countIntervalsLevel) && !is.null(input$countIntervalsGroup) && !is.null(input$countIntervalsscale)){
    plotIntervals(x = dataFilt(), level = input$countIntervalsLevel, 
                  interval_scale=input$countIntervalsscale,
                  colorBy = input$countIntervalsGroup, grouped=TRUE, 
                  facetBy=input$countIntervalsFacet, 
                  calculation_type = input$countIntCalType,
                  show_stats=input$countIntervalsshowstats, label_colors = NULL)
...
if(!is.null(input$dissimilarityLevel) && !is.null(input$dissimilarityLevel) && !is.null(input$dissimilarityIndex) && !is.null(input$dissimilarityClustering) && !is.null(input$multdissGroup) && input$doHm1 == 1){
     plotDissHeatmap(x = dataFilt(), level = input$dissimilarityLevel, method = input$dissimilarityIndex, binary = FALSE, clustering = input$dissimilarityClustering, 
                       label_colors = NULL, annotation_groups =input$multdissGroup)
}
...
if(!is.null(input$MDSLevel) && !is.null(input$MDSMethod) && !is.null(input$multMDSGroup)){
    plotDissMDS(x = dataFilt(), level = input$MDSLevel, method = input$MDSMethod,  label_colors = NULL, colorBy = input$multMDSGroup, shapeBy= input$multMDSShape)
}
