...
```{r title, echo = FALSE}
  tags$body(style="color: white;",
       p(style="color: white;",
  textInput("title", "Enter title", value="")))
```
...
if((!is.null(input$statisticsStat) && !is.null(input$statisticsGroup) && input$doStats == 1) || 
   (!is.null(input$diverLevel) && !is.null(input$diverGroup) && !is.null(input$diverIndex) && input$doDiversity == 1) || 
   (!is.null(input$multrenLevel) && !is.null(input$multrenGroup)) || 
   (!is.null(input$countIntervalsLevel) && !is.null(input$countIntervalsGroup) && input$doCountInt == 1) || 
   (!is.null(input$multRankLevel) && !is.null(input$multRankScale) && !is.null(input$multRankGroup)) || 
   (!is.null(input$geneUsageLevel) && !is.null(input$geneUsageScale) && !is.null(input$geneUsageGroup) && input$dogeneUsage == 1)){cat("## Comparison of basic statistics")}
```


```{r show Metadata statistics multiple title, results='asis', echo=FALSE}
if(!is.null(input$statisticsStat) && !is.null(input$statisticsGroup) && input$doStats == 1){cat("### Metadata statistics")}
```
...
if(!is.null(input$diverLevel) && !is.null(input$diverGroup) && !is.null(input$diverIndex) && input$doDiversity == 1){cat("### Diversity indices")}
```
...
if(!is.null(input$countIntervalsLevel) && !is.null(input$countIntervalsGroup) && input$doCountInt == 1){cat("### Count intervals")}
```
...
if(!is.null(input$geneUsageLevel) && !is.null(input$geneUsageScale) && !is.null(input$geneUsageGroup) && input$dogeneUsage == 1){cat("### V/J usage")}
```
...
if(!is.null(input$geneUsageLevel) && !is.null(input$geneUsageScale) && !is.null(input$geneUsageGroup) && input$dogeneUsage == 1){
